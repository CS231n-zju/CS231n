...
def affine_forward(x, w, b):
    ...
    num_train, input_dim = x.shape[0], x.size / x.shape[0]

    # print num_train,input_dim
    # print x.shape
    # print w.shape

    out = np.dot(x.reshape(num_train, input_dim), w) + b
...
def affine_backward(dout, cache):
    ...
    #############################################################################
    pass
    num_train, input_dim = x.shape[0], w.shape[0]

    # print num_train,input_dim   10 6
    # print 'x:',x.shape       x: (10L, 2L, 3L)
    # print 'w:',w.shape       w: (6L, 5L)

    dx = np.dot(dout, w.T).reshape(x.shape)
    dw = np.dot(x.reshape(num_train, input_dim).T, dout)
...
def relu_forward(x):
    ...
    #############################################################################
    out = np.maximum(0, x)  # also could be code as : out=(x>0)*x
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = x
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    output_weight = (W + 2 * pad - WW) / stride + 1
    output_height = (H + 2 * pad - HH) / stride + 1

    x_padded = np.pad(x, ((0,), (0,), (pad,), (pad,)), 'constant')
    out = np.zeros((N, F, output_height, output_weight))
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(output_height):
        for j in range(output_weight):
            x_pad_mask = x_paded[:, :, i * stride: i * stride + HH, j * stride:j * stride + WW]
            for k in range(F):
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    out_heigth = H / pool_height
    out_width = W / pool_width
    out = np.zeros((N, C, out_heigth, out_width))
